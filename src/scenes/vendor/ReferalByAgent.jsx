...
import { referalTableColumns } from "../../custom/TableColumns";
...
export default function ReferalByAgent() {
  ...
  const fetchAllUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE_URL}/user/agent/${profile?._id}/labours`,
        {
          headers: {
...
console.log(response.data.data)
  ...
  const columns = referalTableColumns({ handleView });

  return (
    <Box>
      <Header title="Referred Labours" />
      <Box
        sx={{
          display: "flex",
